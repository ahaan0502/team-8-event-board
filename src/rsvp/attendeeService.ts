...
import type { RSVPRepository, RSVPStatus } from "../events/rsvpRepository";
...
class AttendeeService implements IAttendeeService {
  ...
  async getAttendeeList(
    ...
    const eventResult = await this.eventService.getEventById(eventId, requesterId);
    ...
    const rsvps = await this.rsvpRepo.getByEvent(eventId);
