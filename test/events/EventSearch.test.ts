...
const tsEvent     = makeEvent({ id: "e1", title: "TypeScript Talk", description: "Advanced TS patterns" });
...
const draftEvent  = makeEvent({ id: "e4", status: "draft", title: "Hidden Draft" });

const service = CreateEventService(makeRepo([tsEvent, jazzEvent, campusEvent, draftEvent]));

describe("EventService search (Feature 10: q filter)", () => {
  it("returns all published upcoming events when query is empty", async () => {
    const result = await service.listEvents({ q: "" });
  ...
  it("matches by title (case-insensitive)", async () => {
    const result = await service.listEvents({ q: "typescript" });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.map((e) => e.id)).toEqual(["e1"]);
  });

  it("matches by description", async () => {
    const result = await service.listEvents({ q: "live jazz" });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("matches by location", async () => {
    const result = await service.listEvents({ q: "campus" });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("returns empty list when no events match", async () => {
    const result = await service.listEvents({ q: "xyznonexistent" });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("does not return draft events even if they match the query", async () => {
    const result = await service.listEvents({ q: "hidden draft" });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("returns InvalidSearchError for a query over 200 characters", async () => {
    const result = await service.listEvents({ q: "a".repeat(201) });
  ...
  it("accepts a query of exactly 200 characters", async () => {
    const result = await service.listEvents({ q: "a".repeat(200) });
    expect(result.ok).toBe(true);
  });
});
