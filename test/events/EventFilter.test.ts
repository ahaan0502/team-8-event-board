...
describe("EventService.listEvents (Feature 6: category and date filter)", () => {
  ...
  it("returns results sorted by start time ascending", async () => {
    const result = await service.listEvents({});
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("does not return draft events even when no filter is applied", async () => {
    const result = await service.listEvents({});
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.value.map((e) => e.id)).not.toContain("e3");
  });
});
